...
def load_model_from_checkpoint(problem, model_args, device):
    ...
    if model_path is not None:
        ...
        # Add this section to handle DataParallel state dict
        if list(state_dict["net"].keys())[0].startswith("module."):
            # Remove 'module.' prefix if present
            new_state_dict = {k.replace("module.", ""): v for k, v in state_dict["net"].items()}
